...
int tmpNum = 0;
int labelNum = 0;
// currentOffset and dummyCurrentOffset are defined in symbolTable.c
// extern declarations are in symbolTableDef.h
...
intermed * generateIRCode(astNode * currentNode, quad * labels, tableStack * tbStack)
{
    ...
    else if(!strcmp(currentNode->node->ele.internalNode->label, "SWITCH"))
    {
        ...
        while(trav!=NULL)
        {
            ...
            else
            {
                ...
                if(!strcmp(trav->child->node->ele.leafNode->type, "TRUE"))
                {
                    initQuad(ifcode->ele, idname, "1\0","if\0");
                    ifcode->ele->tag1 = ID;    
                    ifcode->ele->tag2 = NUM;
                }
                else if(!strcmp(trav->child->node->ele.leafNode->type, "FALSE"))
                {
                    initQuad(ifcode->ele, idname, "0\0","if\0");
                    ifcode->ele->tag1 = ID;    
                    ifcode->ele->tag2 = NUM;
                }
                else
    ...
    else if(!strcmp(currentNode->node->ele.internalNode->label, "ID_ARR"))
    {
        ...
        if(currentNode->child->sibling == NULL)
        {
            ...
            else
            {
                ...
                strcpy(ifcode->ele->op, "==\0");
                initQuad(ifcode->ele, currentNode->child->node->ele.leafNode->lexeme, "1\0","if\0");
                ifcode->ele->tag1 = ID;
                ifcode->ele->tag2 = NUM;
        ...
        else
        {
            ...
            if(labels != NULL)
            {
                ...
                strcpy(ifcode->ele->op, "==\0");
                initQuad(ifcode->ele, final->t.name, "1\0","if\0");
                ifcode->ele->tag1 = ID;
                ifcode->ele->tag2 = NUM;
    ...
    else if(!strcmp(currentNode->node->ele.internalNode->label, "MODULECALL"))
    {
        ...
        // set trav to be id_list for output to the function
    ...
    else if(!strcmp(currentNode->node->ele.internalNode->label, "DECLARE"))
    {
        ...
        if(!strcmp(currentNode->child->sibling->node->ele.internalNode->label,"ARRAY"))
        {
            ...
            while(temp!=NULL)
            {
                ...
                // symbolTableNode * rest = sym_hash_find(temp->node->ele.leafNode->lexeme, &(tbStack->top->ele->hashtb),0,NULL);
                // rest->aux = 2;
        ...
        else
        {
            ...
            while(temp != NULL)
            {
                // symbolTableNode * ret = sym_hash_find(temp->node->ele.leafNode->lexeme, &(tbStack->top->ele->hashtb),0,NULL);
                // ret->aux = 2;
                IRcode * dec = malloc(sizeof(IRcode));
                dec->ele = (quad *)malloc(sizeof(quad));
                dec->next=NULL;
