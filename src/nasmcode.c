...
IRcode* nasmRecur(IRcode* code, tableStack* tbStack, symbolTable * symT, FILE * fp)
{
    ...
    // Add GNU-stack section at the end to prevent executable stack warning
    // This should be written only once when we're at the top level
    if(tbStack->size == 1)
    {
        FILE *fp_temp = fp;
        fprintf(fp_temp, "\nSECTION .note.GNU-stack noalloc noexec nowrite progbits nobits\n");
    }
}
